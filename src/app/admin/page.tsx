  ...
  <div className="bg-gray-200 rounded-2xl shadow-xl p-8">
    ...
    <div className="grid gap-6 max-w-3xl mx-auto mt-8">
      {posts.slice(0, 5).map((post: Post) => (
        <div key={post.id} className="bg-blue-500 transition-colors duration-500 ease-in-out hover:bg-blue-950 p-6 rounded-xl shadow-md">
          <PageCard post={post} />
          {isAdmin && (
            <div className="flex gap-2 mt-4">
              <button className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 transition">Edit</button>
